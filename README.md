...
- Install only on a TV you own, with existing owner-controlled root and Unknown Core 0.7.0 or newer. No rooting exploit, LG credentials, firmware or service-login bypass is supplied.
...
Output: dist/store-1.1.0.zip and dist/SHA256SUMS.txt. PC architecture is independent of the TV JavaScript runtime. The initial reviewed candidate ZIP and its checksum are also at the repository root. Checksums verify bytes, not publisher identity or safety.
